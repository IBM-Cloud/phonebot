...
function CallManager (client, channel) {
  ...
  // Find first account number to use as outgoing number
  var that = this
  client.incomingPhoneNumbers.list(function (err, data) {
    var err_message = 'ERROR: Unable to find outgoing phone number from Twilio'
    if (err) throw new Error(err_message + '\n' + JSON.stringify(err))

    var first = data.incoming_phone_numbers[0]
    if (!first) throw new Error(err_message)

    that.from = first.phone_number
  })
}
...
CallManager.prototype.update_call_status = function (details) {
  ...
  switch (status) {
    ...
    case 'canceled':
      ...
      break
    default:
      log.error('ERROR: Unknown call state encountered (' + status + '): ' + JSON.stringify(details))
      status = null
  }
...
CallManager.prototype.call = function (number, route) {
  ...
  this.active_call = {sid: null, status: null, number: number}
...
CallManager.prototype.hangup = function () {
  ...
  this.client.calls(this.active_call.sid).update({
    ...
    that.request_success('Call Terminated: ' + that.active_call, call)
  })
}
...
CallManager.prototype.options = function (opts) {
  ...
  if (opts.hasOwnProperty('verbose')) {
    this.defaults.verbose = opts.verbose
  }
}
